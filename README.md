...
<h3>ENTER YOUR NAME: javan Rufus J</h3>

<h3>ENTER YOUR REGISTER NO : 212224230104</h3>
...
i) Using the program sumArraysOnGPU-timer.cu, set the block.x = 1023. Recompile and run it. Compare the result with the block's execution configuration.x = 1024. Try to explain the difference and the reason for it.

ii) Refer to sumArraysOnGPU-timer.cu, and let block.x = 256. Create a new kernel that lets each thread handle two elements. Compare the results with other execution configurations.
...
4. Allocate memory on the device for input and output arrays, and copy input data from the host to the device.
...
<img width="862" height="178" alt="image" src="https://github.com/user-attachments/assets/2ee0a9a1-1c1d-42fc-b5b0-4212999372ba" />

---

## RESULT:
Thus, the implementation of sum arrays on the host and device is done in nvcc CUDA using random numbers.
